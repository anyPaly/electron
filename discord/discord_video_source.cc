...
void MediaStreamDiscordVideoSource::VideoSourceDelegate::
  ...
  const gpu::SharedImageInfo si_info(
      ...
      kPremul_SkAlphaType, usage, "DiscordVideoSource");
  auto client_shared_image = sii->CreateSharedImage(
      buffer_impl.get(), gpu_factories_->GpuMemoryBufferManager(),
      gfx::BufferPlane::DEFAULT, si_info);
  ...
  mailbox_holder_array[0] = gpu::MailboxHolder(
      ...
      client_shared_image->GetTextureTarget(gfx::BufferUsage::GPU_READ,
                                            buffer_impl->GetFormat()));
  ...
  PostCrossThreadTask(
      *video_task_runner_.get(), FROM_HERE,
...
MediaStreamDiscordVideoSource::MediaStreamDiscordVideoSource(
    ...
    : MediaStreamVideoSource(base::SingleThreadTaskRunner::GetCurrentDefault()),
      stream_id_(streamId) {}
